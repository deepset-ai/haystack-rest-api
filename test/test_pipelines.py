...
def test_run_pipeline_generic_wrong_name(client):
    response = client.post(
        url="/pipelines/non_existing/run", data=json.dumps({"data": {"value": "test"}, "parameters": {}})
    )
...
def test_run_pipeline_generic_wrong_payload_structure(client):
    response = client.post(url="/pipelines/haystack_pipeline_1/run", data=json.dumps({"not": "correct"}))
    assert 422 == response.status_code


def test_run_pipeline_generic_wrong_payload_content(client):
    response = client.post(
        url="/pipelines/haystack_pipeline_1/run", data=json.dumps({"data": {"unexpected": "test"}, "parameters": {}})
    )
...
def test_run_pipeline_generic_correct_payload_data_only(client):
    response = client.post(
        url="/pipelines/haystack_pipeline_1/run", data=json.dumps({"data": {"value": 1}, "parameters": {}})
    )
    assert 200 == response.status_code
    response_json = response.json()
    ...
    response = client.post(
        url="/pipelines/haystack_pipeline_1/run", data=json.dumps({"data": {"value": 10}, "parameters": {}})
    )
    assert 200 == response.status_code
    response_json = response.json()
...
def test_run_pipeline_generic_correct_payload_data_and_params_for_nonexisting_node(client):
    response = client.post(
        url="/pipelines/haystack_pipeline_1/run",
